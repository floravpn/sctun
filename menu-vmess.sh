...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/floravpn/sctun/main/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/floravpn/sctun/main/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/floravpn/sctun/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
echo -e "$COLOR1 ${NC} ${WH}Limit Ip       ${COLOR1}: ${WH}{iplimit} Login" | tee -a /etc/log-create-user.log
echo -e "$COLOR1 ${NC} ${WH}Limit Quota    ${COLOR1}: ${WH}{Quota} GB" | tee -a /etc/log-create-user.log
...
echo -e "$COLOR1 ${NC} ${WH}Port Alternatif${COLOR1}: ${WH}8080,2082,8880,2052" | tee -a /etc/log-create-user.log
